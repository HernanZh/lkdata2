  ...
  derived_table: {
    sql: -- SELECT date,ROUND(SUM(banner_revenue)+SUM(interstitial_revenue)+SUM(rewarded_revenue),2) as sum_revenue
      ...
      SELECT  suffix as date,
        ...
        COALESCE(SUM(( (select value.double_value from UNNEST(events.event_params) where value.double_value IS NOT NULL AND (suffix between '20210820' AND '20210827')AND event_name='ad_revenue_banner')  ) ), 0) AS banner_revenue,
        COALESCE(SUM(( (select value.double_value from UNNEST(events.event_params) where value.double_value IS NOT NULL AND (suffix between '20210820' AND '20210827')AND event_name='ad_revenue_interstitial')  ) ), 0) AS interstitial_revenue,
        COALESCE(SUM(( (select value.double_value from UNNEST(events.event_params) where value.double_value IS NOT NULL AND (suffix between '20210820' AND '20210827')AND event_name='ad_revenue_rewarded')  ) ), 0) AS rewarded_revenue,
        COALESCE(SUM(( (select value.double_value from UNNEST(events.event_params) where value.double_value IS NOT NULL AND (suffix between '20210820' AND '20210827'))  ) ), 0) AS events_sum_revenue,
        FROM `lk-datawarehouse-2.rc_firebase.rc_events` events
        -- UNNEST(user_properties) as userProperty
        WHERE (suffix BETWEEN '20210820' and '20210827')
      ...
      AND events.created_at < (TIMESTAMP('2021-08-28 00:00:00'))
