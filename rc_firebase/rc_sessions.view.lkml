      ...
      SELECT *,
        ...
        WHERE
          ...
          --limits to this year only to save on query costs, customize this as needed
            (((TIMESTAMP(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'\d\d\d\d\d\d\d\d'))) ) >= ((TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), WEEK)))
            AND (TIMESTAMP(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'\d\d\d\d\d\d\d\d'))) ) < ((TIMESTAMP(CONCAT(CAST(DATE_ADD(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), WEEK) AS DATE), INTERVAL 2 WEEK) AS STRING), ' ', CAST(TIME(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), WEEK) AS TIMESTAMP)) AS STRING)))))))
...
SELECT user_pseudo_id,
  ...
  FROM ${rc_last.SQL_TABLE_NAME} as final
       ) session
 GROUP BY 1,2,3
 ;;
  }
