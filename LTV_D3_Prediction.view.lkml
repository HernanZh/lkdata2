datagroup: prediction_trigger_d3 {
  sql_trigger: SELECT MAX(id) FROM etl_log;;
  max_cache_age: "1 hour"
}

view: training_input_d3 {
  derived_table: {
...
view: future_revenue_model_d3 {
  derived_table: {
    datagroup_trigger: prediction_trigger
    sql_create:
      CREATE OR REPLACE MODEL ${SQL_TABLE_NAME}
      OPTIONS(model_type='linear_reg'
        , input_label_cols=['revenue_d3']
        ) AS
      SELECT
         * EXCEPT(date, bundle_id,platform)
      FROM ${training_input_d3.SQL_TABLE_NAME};;
  }
}

########################################## PREDICT FUTURE ############################

view: future_input_d3 {
  derived_table: {
    explore_source: reporting_cohorted_revenue {
      column: date { field:reporting_cohorted_revenue.date}
      column: bundle_id {field: apps.bundle_id}
...
view: future_revenue_prediction_d3 {
  derived_table: {
    sql: SELECT * FROM ml.PREDICT(
          MODEL ${future_revenue_model_d3.SQL_TABLE_NAME},
          (SELECT * FROM ${future_input_d3.SQL_TABLE_NAME}));;
  }
  #measure: predicted_revenue_d7{type:sum}
  measure: revenue_d1{type: sum}
  measure: predicted_revenue_d3{type: sum}
